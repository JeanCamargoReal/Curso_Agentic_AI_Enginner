...
pesquisador_llm = ChatOpenAI(api_key=api_key, model="gpt-4o", temperature=0.7)

# Define o LLM do Agente Assistente Jurídico
assistente_juridico_llm = ChatOpenAI(api_key=api_key, model="gpt-4o", temperature=0.2)

# Define o LLM do Agente Criador de Relatórios
criador_relatorio_llm = ChatOpenAI(api_key=api_key, model="gpt-4o", temperature=0.5)
...
pesquisador = Agent(
    ...
    e pesquisa de mercado. Você tem Pós-Graduação em Ciência de Dados pela Data Science Academy e é
...
def dsa_execute_time_agentes(topic):
    try:
        ...
        time_juridico = Crew(
            agents=[pesquisador, assistente_juridico, criador_relatorio],
            tasks=tasks,
            verbose=True,
        )
